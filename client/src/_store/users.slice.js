...
function createExtraActions() {
  ...
  function getAll() {
    return createAsyncThunk(
      `${name}/getAll`,
      async () => await fetchWrapper.get(`${baseUrl}/auth/all-user`)
    );
  }
  ...
  function getById() {
    return createAsyncThunk(
      `${name}/getById`,
      async (id) => await fetchWrapper.get(`${baseUrl}/auth/profile/${id}`)
    );
  }
  ...
  function update() {
    return createAsyncThunk(
      `${name}/update`,
      async ({ id, data }) =>
        await fetchWrapper.put(`${baseUrl}/auth/user-profile/${id}`, data)
  ...
  function forgotPassword() {
    return createAsyncThunk(
      "user/forgotPassword",
      async (emailData) =>
        await fetchWrapper.post(`${baseUrl}/auth/forgot-password`, emailData)
    );
  }
  function resetPassword() {
    return createAsyncThunk(
      ...
      async ({ resetToken, password }) =>
        await fetchWrapper.post(`${baseUrl}/auth/reset-password-update`, {
          resetToken,
          password,
        })
    );
